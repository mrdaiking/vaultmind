...
async def get_management_api_token() -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 200:
            logger.error(f"[ERROR]❌ Auth0 Management API token request failed!")
            logger.error(f"[ERROR]   Status: {response.status_code}")
            logger.error(f"[ERROR]   Response: {response.text}")
            logger.error(f"[ERROR]")
            logger.error(f"[WARNING]⚠️  ACTION REQUIRED:")
            logger.error(f"[WARNING]   1. Go to Auth0 Dashboard → Applications → APIs")
            logger.error(f"[WARNING]   2. Click 'Auth0 Management API'")
            logger.error(f"[WARNING]   3. Go to 'Machine to Machine Applications' tab")
            logger.error(f"[WARNING]   4. Find your app (VaultMind) and TOGGLE IT ON ✅")
            logger.error(f"[WARNING]   5. Expand dropdown and check: read:users, read:user_idp_tokens")
            logger.error(f"[WARNING]   6. Click 'Update'")
        ...
        _mgmt_token_cache["expires_at"] = datetime.now() + timedelta(
...
async def get_google_access_token_from_management_api(user_sub: str) -> Optional[str]:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                logger.error(
                    f"[ERROR] Failed to fetch user from Management API: {response.status_code} {response.text}"
                )
                return None
            ...
            for identity in identities:
                if identity.get("provider") == "google-oauth2":
                    ...
                    if access_token:
                        ...
                        logger.warning(f"[WARNING]❌ Google identity found but no access_token property")

            logger.warning(f"[WARNING]❌ No Google identity found in Management API response")
            return None

    except Exception as e:
        logger.error(f"[ERROR] Error fetching Google token from Management API: {e}")
        return None
